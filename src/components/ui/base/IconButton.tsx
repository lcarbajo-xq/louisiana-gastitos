...
export const IconButton: React.FC<IconButtonProps> = ({
  ...
  const getVariantStyles = () => {
    ...
    switch (variant) {
      ...
      case 'secondary':
        return `${baseStyles} bg-gray-700 active:bg-gray-800 border border-gray-600 ${
          disabled ? 'bg-gray-800 border-gray-700' : ''
        }`
      case 'ghost':
        return `${baseStyles} bg-transparent active:bg-gray-800 ${
          disabled ? 'bg-transparent' : ''
        }`
      case 'danger':
        return `${baseStyles} bg-red-600 active:bg-red-700 ${
          disabled ? 'bg-gray-600' : ''
        }`
      default:
        return `${baseStyles} bg-blue-600 active:bg-blue-700`
